...
1. What is the purpose of using _sessions_?
  To persist state information between page load since the http request does not persist information between client and server.
1. What does bcrypt do to help us store passwords in a secure manner.
  It irreversibly convert (hash) the password into a fixed length of string. To make the hashing more secure, bcrypt allows you to add custom salt, that is a secret string which will be appended to the original password prior to hashing, or it will provide a default salt.
1. What does bcrypt do to slow down attackers?
  It add salt and rounds (hashing repetation) so that it would take more time for the attacker to run brute force attack.
